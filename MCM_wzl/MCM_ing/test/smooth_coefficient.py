...
def run(surface_sample):
	amap = np. zeros((512,512))
	clear()
	maximum(surface_sample, amap, top, bottom)
	amap = np. zeros((512,512))
	surface_solver(surface_sample, amap, top)
	surface_solver(surface_sample, amap, bottom)
	return float(len(summ))/512./512.